...
int main( int argc, char* args[] )
{
    ...
    _BS_sound_device = new BS_Sound();
	if(!_BS_sound_device->get_OpenAL_status())
	{
...
void play_music_continuosly()
{
	...
	if(muscicIndex == 1 && !_BS_sound_device->isPlaying(BS_Available_Sounds::melody_1()))
	{
		_BS_sound_device->play_sound(BS_Available_Sounds::melody_2());
		muscicIndex = 2;
	}
	else
        if(muscicIndex == 2 && !_BS_sound_device->isPlaying(BS_Available_Sounds::melody_2()))
        {
            _BS_sound_device->play_sound(BS_Available_Sounds::melody_3());
            muscicIndex = 3;
        }
        else
            if(muscicIndex == 3 && !_BS_sound_device->isPlaying(BS_Available_Sounds::melody_3()))
            {
                _BS_sound_device->play_sound(BS_Available_Sounds::melody_1());
                muscicIndex = 1;
            }
}
...
//    }
    ...
    if(!isPaused)
    {
        ...
        {
            if(!_BS_sound_device->isPlaying(BS_Available_Sounds::power_up()))
            {
                _BS_sound_device->play_sound(BS_Available_Sounds::power_up());
            }
        }
    }
    ...
    {
	    _BS_sound_device->stop_playing_sound(BS_Available_Sounds::power_up());
    }

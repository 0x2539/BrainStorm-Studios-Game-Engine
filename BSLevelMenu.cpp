...
void BSLevelMenu::loadLevels(std::string chapterName, ChapterLevelsStructure* theLevelsToAdd[], int nrOfLevels, int chapterIndex)
{
	...
	doc->readAnyXML("The Levels/playerDetails" + BS_ToString(chapterIndex + 1) + ".xml", "Chapter_of_level", "Level_details",
	...
	while(i < nrOfLevels)
	{
		...
		theLevelsToAdd[i]->textOnLevel =  BS_ToString(i + 1);

		chapterName = "Chapter" + BS_ToString(atof(levelDetailsAttributes[0].c_str())); /// BS_ToString(atof(levelDetails[i]->theCharArray2DAttributes[0]));
		...
		if(i == 0)
		{
			...
			loadATexture(name1Char, theLevelsToAdd[0]->textureOfConnectionLeftRight);
			loadATexture(name2Char, theLevelsToAdd[0]->textureOfConnectionUpDown);
			loadATexture(name3Char, theLevelsToAdd[0]->textureOfFinishedLevel);
			loadATexture("Menu Of Chapters/buttons menu background.png", theLevelsToAdd[0]->textureOfLevel);
		}
	...
	loadATexture(levelChooserTextureNameChar, levelChoserPlayer->texture);
...
void BSLevelMenu::loadChapters()
{
	...
	for(int i = 0 ; i < nrOfChapters ; i++)
	{
		...
		chaptere[i]->textOnChapter =  BS_ToString(i + 1);
		...
		std::string textureName = "Menu Of Chapters/The Chapters textures/chapterTexture" + BS_ToString(i + 1) + ".png";
		loadATexture(textureName.c_str(), chaptere[i]->texture);

		levelChoserPlayer->indexOfChapterThatItIsOn = 0;

		loadATexture("Menu Of Chapters/connectionLeftRight.png", chaptere[i]->levelFromChapter[0]->textureOfConnectionLeftRightChapter);

		loadATexture("Menu Of Chapters/connectionUpDown.png", chaptere[i]->levelFromChapter[0]->textureOfConnectionUpDownChapter);
...
void BSLevelMenu::loadUserData()
{
	for(int i = 0; i < 6; i++)
	{
		std::string file = "Saves/playerData" + BS_ToString(i + 1) + ".txt";

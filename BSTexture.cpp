...
//	mouseTexture->yCord = hero->theBody->GetPosition().y;
	...
	for(int i = 0 ; i < nrMeniuri ; i++)
	{
		...
		for(int j = 0 ; j < nrButtons ; j++)
		{
			...
			loadATexture("Button images/New Buttons/" + namesOfMenus + "/The Unnamed Buttons/picture" + BS_ToString(j) + ".png", newButton->textureOfButtons[0]);
			loadATexture("Button images/New Buttons/" + namesOfMenus + "/The Unnamed Buttons/picture0" + BS_ToString(j) + ".png", newButton->textureOfButtons[1]);
...
//renderMouseTexture();
	...
	if(!DarkLevels)
	{
		...
		renderMyFont(BS_ToString(coinsTotalNo + coinsThisLevel), originalCoin->xCord + CameraPosition->xCord - CameraExtremeLeft + 1,
	...
	for(int it = 0; it < listOfCoins.size(); it++)
	{
		...
		else
		{
			...
			if(listOfCoins[it]->sinValue > 0)
			{
				...
				listOfCoins[it]->color.a = sin(BS_AngleToRadians(listOfCoins[it]->sinValue));
				listOfCoins[it]->sinValue2 += 0.5f;
				listOfCoins[it]->yCord += sin(BS_AngleToRadians(listOfCoins[it]->sinValue2));
		...
		{
			listOfCoins[it]->color.r = 1 + sin(BS_AngleToRadians(redSinForCoins));
		...
		if(script->xPower)
		{
			...
			if(xCord < maxRight && script->value1 > 0)
			{
				...
				if(xCord > maxRight - 1)
				{
					...
					{
						if(sin(BS_AngleToRadians(script->value2)) > 0)
						{
							script->value2 = 90;
						}
						else
			...
			xSpeed = script->xPower * sin(BS_AngleToRadians(script->value2));
		}
		else
			if(script->yPower)
			{
				...
				ySpeed = script->yPower * sin(BS_AngleToRadians(script->value2));
			}
	}
...
void BSTexture::renderDeadSceneChange()
{
	...
	deadScene->color.a = sin(BS_AngleToRadians(deadScene->SinValueColorOfTheScene));
...
void BSTexture::renderNextLevelSceneChange()
{
	if(deadScene->CosValueColorOfTheScene < 90 && deadScene->colorGoesUpOrDown == 1)
	{
		...
		if(deadScene->CosValueColorOfTheScene >= 90 && deadScene->colorGoesUpOrDown == 1)
		{
			...
			addObstacle->incarcaHarta("Menu Of Levels/Chapter" + BS_ToString(levelChoserPlayer->indexOfChapterThatItIsOn + 1) + "/The Levels objects/level" +
			                          BS_ToString(levelChoserPlayer->indexOfLevelThatItIsOn + 1) + ".xml");
	...
	deadScene->color.a = sin(BS_AngleToRadians(deadScene->CosValueColorOfTheScene));
	deadScene->xCord = CameraPosition->xCord - CameraExtremeLeft;
	deadScene->yCord = CameraPosition->yCord - CameraExtremeDown;
	deadScene->zCord = scaleSizeInitial;
	...
	if(deadScene->CosValueColorOfTheScene < 90 && deadScene->colorGoesUpOrDown == 1)
	{
		...
		if(deadScene->CosValueColorOfTheScene >= 90 && deadScene->colorGoesUpOrDown == 1)
		{
			...
			std::string nameOfLevel = "Menu Of Levels/Chapter" +
			                          BS_ToString(levelChoserPlayer->indexOfChapterThatItIsOn + 1) + "/The Levels details/level" +
			                          BS_ToString(levelChoserPlayer->indexOfLevelThatItIsOn + 1) + ".txt";
...
void BSTexture::renderDustBodies()
{
	for(int i = 0; i < ListOfSquareDustBodies.size(); i++)
	{
		...
		else
		{
			...
			{
				decal = 2 * sin(BS_AngleToRadians(ListOfSquareDustBodies[i]->value1 - 30));
			}
			color.a = cos(BS_AngleToRadians(ListOfSquareDustBodies[i]->value1));
		}
...
void BSTexture::renderAnyButtons(BSMenuStructure *theMenu)
{
	...
	for(int i = 0; i < theMenu->listOfButtons.size(); i++)
	{
		...
		float newW = (float) currentButton->width + 1 + sin(BS_AngleToRadians(currentButton->value1));
		float newH = (float) currentButton->height + (1 + sin(BS_AngleToRadians(currentButton->value2))) * whRatio;
...
void BSTexture::loadMenuButtonsTexture(int numberOfButtons, std::string pathAndName, GLuint theButtonsTexture[])
{
	...
	{
		std::string TextureName = pathAndName + BS_ToString(i + 1) + ".png";
...
{
	double lineHeight = hero->width;
//    FontSizeX = 1;
//    FontSizeY = 1;
	BSColorRGBA newColor = color;
	float pozNou = xCord, newY = yCord;
	float lastWidth = 0, lastX = pozNou;
	float xSize = FontSizeX, ySize = FontSizeY, newWidth = 1;
	...
	for(unsigned int i = 0 ; i < theStringToRenderString.size() ; i++)
	{
		...
		{
			xSize = FontSizeX, ySize = FontSizeY, newWidth = 1;
			let = theStringToRenderString[i];
...
//				std::cout<<theStringToRenderString[i]<<' '<<letters[theStringToRenderString[i]]->widthOfLetterOnScreen<<' '<<letters[theStringToRenderString[i]]->heightOfLetterOnScreen<<"; ";
...
//		ListaDeCaractere *nodesForXML[1000];
	...
	std::string theNameOfMap = "Menu Of Levels/Chapter" + BS_ToString(levelChoserPlayer->indexOfChapterThatItIsOn + 1)
	                           + "/The Levels objects/level" + BS_ToString(levelChoserPlayer->indexOfLevelThatItIsOn + 1)
	                           + ".xml";
	///Salvez jucatorul
	ListaDeCaractere *heroLine = new ListaDeCaractere;

	heroLine->theCharArray2D.push_back("0");
	heroLine->theCharArray2D.push_back("playerTexture.png");
	heroLine->theCharArray2D.push_back("0");
	heroLine->theCharArray2D.push_back(BS_ToString(HeroInitialX));
	heroLine->theCharArray2D.push_back(BS_ToString(HeroInitialY));
	heroLine->theCharArray2D.push_back(BS_ToString(Hero_size));
	heroLine->theCharArray2D.push_back(BS_ToString(Hero_size));
	heroLine->theCharArray2D.push_back("hero");
	heroLine->theCharArray2D.push_back("playerAnimation");
	heroLine->theCharArray2D.push_back("0");
	...
	std::string string0, string1, string2, string3, string4, string5, string6, string7, string8, string9;
	...
	for(int i = 0; i < ListOfRoundBodies.size(); i++)
	{
		...
		string1 = ListOfRoundBodies[i]->nameOfTexture;
		...
		{
			string2 = BS_ToString(ListOfRoundBodies[i]->ObstacleCanKillHero);
		}

		if(ListOfRoundBodies[i]->script)
		{
			string0 = "9";
			string3 = BS_ToString(ListOfRoundBodies[i]->script->xInitial);
			string4 = BS_ToString(ListOfRoundBodies[i]->script->yInitial);;
		}
		else
		{
			string0 = "1";
			string3 = BS_ToString(ListOfRoundBodies[i]->theBody->GetPosition().x);
			string4 = BS_ToString(ListOfRoundBodies[i]->theBody->GetPosition().y);
		}

		string5 = BS_ToString(ListOfRoundBodies[i]->width);
		string6 = BS_ToString(ListOfRoundBodies[i]->width);
		string7 = salveazaUserData->name;
		string8 = "roundAnimation";
		string9 = BS_ToString(ListOfRoundBodies[i]->index);

		line->theCharArray2D.push_back(string0);
		line->theCharArray2D.push_back(string1);
		line->theCharArray2D.push_back(string2);
		line->theCharArray2D.push_back(string3);
		line->theCharArray2D.push_back(string4);
		line->theCharArray2D.push_back(string5);
		line->theCharArray2D.push_back(string6);
		line->theCharArray2D.push_back(string7);
		line->theCharArray2D.push_back(string8);
		line->theCharArray2D.push_back(string9);
	...
	for(int i = 0; i < ListOfSquareBodies.size(); i++)
	{
		...
		{
			string0 = "8";
			string3 = BS_ToString(ListOfSquareBodies[i]->script->xInitial);
			string4 = BS_ToString(ListOfSquareBodies[i]->script->yInitial);
		}
		else
		{
			string0 = "0";
			string3 = BS_ToString(ListOfSquareBodies[i]->theBody->GetPosition().x);
			string4 = BS_ToString(ListOfSquareBodies[i]->theBody->GetPosition().y);
		}

		string1 = ListOfSquareBodies[i]->nameOfTexture;
		string5 = BS_ToString(ListOfSquareBodies[i]->width);
		string6 = BS_ToString(ListOfSquareBodies[i]->height);
		string7 = salveazaUserData->name;
		string8 = "textureAnimation";
		string9 = BS_ToString(ListOfSquareBodies[i]->index);
		...
		{
			string2 = BS_ToString(ListOfSquareBodies[i]->ObstacleCanKillHero);
		}
		line->theCharArray2D.push_back(string0);
		line->theCharArray2D.push_back(string1);
		line->theCharArray2D.push_back(string2);
	...
	for(int i = 0; i < listOfMessages.size(); i++)
	{
		...
		string0 = "2";
		string1 = listOfMessages[i]->nameOfTextureMessage;
		string2 = "0";
		string3 = BS_ToString(listOfMessages[i]->xCord);
		string4 = BS_ToString(listOfMessages[i]->yCord);
		string5 = "0";
		string6 = "0";
		string7 = "message";
		string8 = "messageAnimation";
		string9 = BS_ToString(listOfMessages[i]->index);
	...
	for(int i = 0; i < listOfCoins.size(); i++)
	{
		...
		string0 = "5";
		string1 = "Animations/coin.png";
		string2 = "0";
		string3 = BS_ToString(listOfCoins[i]->xInit);
		string4 = BS_ToString(listOfCoins[i]->yInit);
		string5 = "1";
		string6 = "1";
		string7 = "coin";
		string8 = BS_ToString(listOfCoins[i]->available);
		string9 = BS_ToString(listOfCoins[i]->index);
	...
	for(int i = 0; i < listOfFans.size(); i++)
	{
		...
		string0 = "6";
		string1 = "Animations/fan.png";
		string2 = "0";
		string3 = BS_ToString(listOfFans[i]->xCord);
		string4 = BS_ToString(listOfFans[i]->yCord);
		string5 = BS_ToString(listOfFans[i]->width);
		string6 = BS_ToString(listOfFans[i]->height);
		string7 = BS_ToString(listOfFans[i]->lenthOfPower) + " " + BS_ToString(listOfFans[i]->powerX) + " " + BS_ToString(listOfFans[i]->powerY);
		string8 = "none";
		string9 = BS_ToString(listOfFans[i]->index);
	...
	for(int i = 0; i < listOfWormHoles.size(); i++)
	{
		...
		string0 = "7";
		string1 = "Animations/wormHole.png";
		string2 = "0";
		string3 = BS_ToString(listOfWormHoles[i]->xCord);
		string4 = BS_ToString(listOfWormHoles[i]->yCord);
		string5 = "0";
		string6 = "0";
		string7 = BS_ToString(listOfWormHoles[i]->indexOfWorm);
		string8 = BS_ToString(listOfWormHoles[i]->connectionIndex);
		string9 = BS_ToString(listOfWormHoles[i]->index);
	...
	for(int i = 0; i < ListOfSquareDustBodies.size(); i++)
	{
		...
		string0 = "10";
		string3 = BS_ToString(ListOfSquareDustBodies[i]->xInit);
		string4 = BS_ToString(ListOfSquareDustBodies[i]->yInit);


		string1 = ListOfSquareDustBodies[i]->nameOfTexture;
		string5 = BS_ToString(ListOfSquareDustBodies[i]->width);
		string6 = BS_ToString(ListOfSquareDustBodies[i]->height);
		string7 = "object";
		string8 = "dust";
		string9 = BS_ToString(ListOfSquareDustBodies[i]->index);
	...
	attributesValues.push_back(BS_ToString(gameBoundLeft));
	attributesValues.push_back(BS_ToString(gameBoundDown));
	attributesValues.push_back(BS_ToString(gameBoundUp));
	attributesValues.push_back(BS_ToString(gameBoundRight));
	attributesValues.push_back(BS_ToString(levelHasScript));

	saveTheMap->writeAnyXML(theNameOfMap, "Level", "obstacle", theNodes, nodesForXML, attributesName, attributesValues);
	delete saveTheMap;
	aSalvatHarta = true;
...
{
	std::string file = "Saves/playerData" + BS_ToString(levelChoserPlayer->indexOfChapterThatItIsOn + 1) + ".txt";

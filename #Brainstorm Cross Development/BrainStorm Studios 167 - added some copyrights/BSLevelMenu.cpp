...
BSTexture *renderLevelClass = new BSTexture();
...
void BSLevelMenu::loadLevels(std::string chapterName, ChapterLevelsStructure* theLevelsToAdd[], int nrOfLevels, int chapterIndex)
{
    ...
    while(i < nrOfLevels)
    {
        ...
        theLevelsToAdd[i]->indexOfLevel = atof(levelDetails[i]->theCharArray2D[2].c_str());///i;
        ...
        chapterName = "Chapter"+BStoString(atof(levelDetailsAttributes[0]->theCharArray2DAttributes[0].c_str()));/// BStoString(atof(levelDetails[i]->theCharArray2DAttributes[0]));
        ...
        if(i == 0)
        {
            ...
            renderLevelClass->loadATexture(name1Char, theLevelsToAdd[0]->textureOfConnectionLeftRight);
            renderLevelClass->loadATexture(name2Char, theLevelsToAdd[0]->textureOfConnectionUpDown);
            renderLevelClass->loadATexture(name3Char, theLevelsToAdd[0]->textureOfFinishedLevel);
            renderLevelClass->loadATexture("Menu Of Chapters/buttons menu background.png", theLevelsToAdd[0]->textureOfLevel);
        }
    ...
    renderLevelClass->loadATexture(levelChooserTextureNameChar, levelChoserPlayer->texture);
...
void BSLevelMenu::loadChapters()
{
	...
	nrOfChapters = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[0].c_str());
    ...
    for(int i = 0 ; i < nrOfChapters ; i++)
    {
        ...
        renderLevelClass->loadATexture(textureName.c_str(), chaptere[i]->texture);

        levelChoserPlayer->indexOfChapterThatItIsOn = 0;

		renderLevelClass->loadATexture("Menu Of Chapters/connectionLeftRight.png", chaptere[i]->levelFromChapter[0]->textureOfConnectionLeftRightChapter);

		renderLevelClass->loadATexture("Menu Of Chapters/connectionUpDown.png", chaptere[i]->levelFromChapter[0]->textureOfConnectionUpDownChapter);

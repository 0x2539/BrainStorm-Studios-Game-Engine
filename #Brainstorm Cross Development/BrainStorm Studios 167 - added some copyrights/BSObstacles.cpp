...
void BSObstacles::incarcaHarta(std::string cale)
{
	...
	doc->readAnyXML(cale.c_str(), "Level", "obstacle", levelDetails, theNodes, theAttributes, levelDetailsAttributes, 10, 6);


    scaleRatio = cross_platform_scale;

	CameraExtremeLeft = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[1].c_str()) + SCREEN_WIDTH / 2.f / scaleRatio;
	CameraExtremeDown = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[2].c_str()) + SCREEN_HEIGHT / 2.f / scaleRatio;
	CameraExtremeUp = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[3].c_str()) - SCREEN_HEIGHT / 2.f / scaleRatio;
	CameraExtremeRight = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[4].c_str()) - SCREEN_WIDTH / 2.f / scaleRatio;

	gameBoundLeft = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[1].c_str());
	gameBoundDown = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[2].c_str());
	gameBoundUp = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[3].c_str());
	gameBoundRight = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[4].c_str());
	...
	levelHasScript = atof(levelDetailsAttributes[0]->theCharArray2DAttributes[5].c_str());
								 ...
								 float width, float height, std::string texturePath)
{
	...
	else if(squareOrCirlce == -1)
	{
		...
		renderObstacleClass->loadATexture(texturePath.c_str(), p->texture);
...
void BSObstacles::addMessageTexture(float xCoord, float yCoord, float width, float height, std::string textureName)
{
	...
	renderObstacleClass->loadATexture(fullTextureName.c_str(), newMessage->textureOfSign);

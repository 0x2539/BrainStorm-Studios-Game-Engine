...
void BSLevelMenu::loadLevels(std::string chapterName, ChapterLevelsStructure* theLevelsToAdd[], int nrOfLevels, int chapterIndex)
{
    ...
    while(i < nrOfLevels)
    {
        ...
        if(i == 0)
        {
            ...
            loadTexture(name1Char, theLevelsToAdd[0]->textureOfConnectionLeftRight);
            loadTexture(name2Char, theLevelsToAdd[0]->textureOfConnectionUpDown);
            loadTexture(name3Char, theLevelsToAdd[0]->textureOfFinishedLevel);
            loadTexture("Menu Of Chapters/buttons menu background.png", theLevelsToAdd[0]->textureOfLevel);
        }
    ...
    loadTexture(levelChooserTextureNameChar, levelChoserPlayer->texture);
...
void BSLevelMenu::loadChapters()
{
    ...
    for(int i = 0 ; i < nrOfChapters ; i++)
    {
        ...
        loadTexture(textureName.c_str(), chaptere[i]->texture);

        levelChoserPlayer->indexOfChapterThatItIsOn = 0;

		loadTexture("Menu Of Chapters/connectionLeftRight.png", chaptere[i]->levelFromChapter[0]->textureOfConnectionLeftRightChapter);

		loadTexture("Menu Of Chapters/connectionUpDown.png", chaptere[i]->levelFromChapter[0]->textureOfConnectionUpDownChapter);

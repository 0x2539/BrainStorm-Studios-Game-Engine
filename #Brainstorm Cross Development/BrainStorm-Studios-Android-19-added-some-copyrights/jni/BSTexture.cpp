...
{
    checkGlError("glLoading1");
	setTheValuesForShaders();
    checkGlError("glLoading2");
...
//	m_up = glm::vec3(0, 1, 0);
	//LOGI("%s", "hello23");
    checkGlError("glLoading3");
    LookAtMatrix = glm::lookAt(m_position, m_position + m_direction, m_up);

    checkGlError("glLoading4");
	//LOGI("%s", "hello24");
	theLoadingImage->width = (float) SCREEN_WIDTH / cross_platform_scale + 1;
	//LOGI("%s", "hello25");
    checkGlError("glLoading5");
	theLoadingImage->height = (float) SCREEN_HEIGHT / cross_platform_scale + 1;
	//LOGI("%s", "hello26");
    checkGlError("glLoading6");
	addObstacle->setTheVaoData(theLoadingImage->vbo, theLoadingImage->vbo, theLoadingImage->ibo,
								theLoadingImage->data, theLoadingImage->width,theLoadingImage->height);

    checkGlError("glLoading7");
	//LOGI("%s", "hello27");
	loadTexture("smb loading.png", theLoadingImage->texture);
	//LOGI("%s", "hello28");
    checkGlError("glLoading8");
	theLoadingImage->color.setColor(1, 1, 1, 1);

    checkGlError("glLoading9");
	//LOGI("%s", "hello29");
}
...
void BSTexture::renderTheLoadingImage(float alphaColor)
{
	...
	//LOGI("%s", "hello32");
    checkGlError("glUniform8");
	glUniform1f(ShaderIsCircle, 0);
	//LOGI("%s", "hello41");
    checkGlError("glUniform11");
	glUniform1f(ShaderCircleRadius, 0);
	//LOGI("%s", "hello42");
    checkGlError("glUniform12");

	theLoadingImage->color.a = alphaColor;

    checkGlError("glUniform13");
	//LOGI("%s", "hello43");
    float scaleRatio = (float)(SCREEN_HEIGHT / 18.0f);

    checkGlError("glUniform14");
	//LOGI("%s", "hello44");
    renderBody((float)SCREEN_WIDTH / scaleRatio / 2.0f + CameraPosition->xCord - CameraExtremeLeft,
									...
									theLoadingImage->vbo, theLoadingImage->width, theLoadingImage->height, theLoadingImage->depth);
...
void BSTexture::initMenuButtons()
{
    ...
    std::string someFile = getFile("Menus/menus details.txt");
    std::stringstream citesteMeniurile;
    citesteMeniurile << someFile;
    //std::string cit;
    //streamer>>cit;
...
//	addObstacle->setTheVaoData(mouseTexture->vbo, mouseTexture->vbo, mouseTexture->ibo, mouseTexture->data, Hero_size / 4.f, Hero_size / 4.f);
//
//	mouseTexture->color.setColor(1, 1, 1, 1);
//
//	mouseTexture->nameOfTexture = "Animations/mouseImage.png";
//	loadTexture(mouseTexture->nameOfTexture, mouseTexture->texture);
...
//	mouseTexture->yCord = hero->theBody->GetPosition().y;
	...
	for(int i = 0 ; i < nrMeniuri ; i++)
	{
		...
		///loadTexture("Menus/first.png", newMenu->textureOfMenu);
		...
		for(int j = 0 ; j < nrButtons ; j++)
		{
			...
			loadTexture("Button images/New Buttons/" + namesOfMenus + "/The Unnamed Buttons/picture" + BStoString(j) + ".png", newButton->textureOfButtons[0]);
			loadTexture("Button images/New Buttons/" + namesOfMenus + "/The Unnamed Buttons/picture0" + BStoString(j) + ".png", newButton->textureOfButtons[1]);

			///Set the data for the menu buttons
			addObstacle->setTheVaoData(newButton->vbo[0], newButton->vbo[0], newButton->ibo[0], newButton->dataOfButtons[0], width * 2.0f, height * 2.0f);
			///Set the data for the menu buttons
			addObstacle->setTheVaoData(newButton->vbo[1], newButton->vbo[1], newButton->ibo[1], newButton->dataOfButtons[1], (width + 0.1f) * 2.0f, (height + 0.1f) * 2.0f);
...
void BSTexture::initMenuLevels()
{
	for(int i = 0 ; i < nrOfChapters; i++)
	{
		...
		addObstacle->setTheVaoData(chaptere[i]->vbo, chaptere[i]->vbo, chaptere[i]->ibo, chaptere[i]->data, chaptere[i]->width, chaptere[i]->height);

		for(int j = 0 ; j < chaptere[i]->nrOfLevels ; j++)
		{
			///Set the Menu Levels
			addObstacle->setTheVaoData(chaptere[i]->levelFromChapter[j]->vboBufferOfLevel, chaptere[i]->levelFromChapter[j]->vboBufferOfLevel,
		...
		addObstacle->setTheVaoData(chaptere[i]->levelFromChapter[0]->vboBufferOfConnectionLeftRight, chaptere[i]->levelFromChapter[0]->vboBufferOfConnectionLeftRight,
		...
		addObstacle->setTheVaoData(chaptere[i]->levelFromChapter[0]->vboBufferOfConnectionUpDown, chaptere[i]->levelFromChapter[0]->vboBufferOfConnectionUpDown,
		...
		addObstacle->setTheVaoData(chaptere[i]->levelFromChapter[0]->vboBufferOfFinishedLevel, chaptere[i]->levelFromChapter[0]->vboBufferOfFinishedLevel,
	...
	///Setez data si vbo al animatiei cu care ma voi plimba prin nivele ca sa il aleg pe cel care vreau
	addObstacle->setTheVaoData(levelChoserPlayer->vbo, levelChoserPlayer->vbo, levelChoserPlayer->ibo,
	...
	addObstacle->setTheVaoData(chaptere[0]->levelFromChapter[0]->vboBufferOfConnectionLeftRightChapter,
	...
	addObstacle->setTheVaoData(chaptere[0]->levelFromChapter[0]->vboBufferOfConnectionUpDownChapter,
	...
	addObstacle->setTheVaoData(chaptere[0]->levelFromChapter[0]->vboBufferOfFinishedLevelChapter,
...
{
	setCameraPosition();
	//CameraPosition->yCord -= 0.05f;
	m_position = glm::vec3(CameraPosition->xCord * cross_platform_scale - SCREEN_WIDTH / 2.0f,
							CameraPosition->yCord * cross_platform_scale - SCREEN_HEIGHT / 2.0f, -CameraPosition->zCord);
    LookAtMatrix = glm::lookAt(m_position, m_position + m_direction, m_up);
//	renderBackgroundNo1();
	renderTheObstacles();
	renderDustBodies();
...
{
	float var = cross_platform_scale;
	...
	float smaller_value = 0.4f;

	//the 2 perpendicular lines
	//horizontal
	renderBody(CameraPosition->xCord - CameraExtremeLeft + SCREEN_WIDTH / var / 2.0f, CameraPosition->yCord - CameraExtremeDown + SCREEN_HEIGHT / var / 2.0f, scaleSizeInitial, 0, 0, 0, theTexture, darkWhite, originalFan->vbo, SCREEN_WIDTH / var, 0.2f, 0);
	//vertical
	renderBody(CameraPosition->xCord - CameraExtremeLeft + SCREEN_WIDTH / var / 2.0f, CameraPosition->yCord - CameraExtremeDown + SCREEN_HEIGHT / var / 2.0f, scaleSizeInitial, 0, 0, 0, theTexture, darkWhite, originalFan->vbo, 0.2f, SCREEN_HEIGHT / var, 0);
	...
	{
		//bottom left
		renderBody(CameraPosition->xCord - CameraExtremeLeft + SCREEN_WIDTH / var / 4.0f, CameraPosition->yCord - CameraExtremeDown + SCREEN_HEIGHT / var / 4.0f, scaleSizeInitial, 0, 0, 0, theTexture, white, originalFan->vbo, SCREEN_WIDTH / var / 2.0f - smaller_value, SCREEN_HEIGHT / var / 2.0f - smaller_value, 0);
	}
	else
		if(quadrantIndex == 2)
		{
			//bottom right
			renderBody(CameraPosition->xCord - CameraExtremeLeft + SCREEN_WIDTH * 3.0f / var / 4.0f, CameraPosition->yCord - CameraExtremeDown + SCREEN_HEIGHT / var / 4.0f, scaleSizeInitial, 0, 0, 0, theTexture, white, originalFan->vbo, SCREEN_WIDTH / var / 2.0f - smaller_value, SCREEN_HEIGHT / var / 2.0f - smaller_value, 0);
		}
	else
		if(quadrantIndex == 3)
		{
			//top left
			renderBody(CameraPosition->xCord - CameraExtremeLeft + SCREEN_WIDTH / var / 4.0f, CameraPosition->yCord - CameraExtremeDown + SCREEN_HEIGHT * 3.0f / var / 4.0f, scaleSizeInitial, 0, 0, 0, theTexture, white, originalFan->vbo, SCREEN_WIDTH / var / 2.0f - smaller_value, SCREEN_HEIGHT / var / 2.0f - smaller_value, 0);
		}
	else
		if(quadrantIndex == 4)
		{
			//top right
			renderBody(CameraPosition->xCord - CameraExtremeLeft + SCREEN_WIDTH * 3.0f / var / 4.0f, CameraPosition->yCord - CameraExtremeDown + SCREEN_HEIGHT * 3.0f / var / 4.0f, scaleSizeInitial, 0, 0, 0, theTexture, white, originalFan->vbo, SCREEN_WIDTH / var / 2.0f - smaller_value, SCREEN_HEIGHT / var / 2.0f - smaller_value, 0);
		}
}
...
void BSTexture::buildRainDrops()
{
	...
	addObstacle->setTheVaoData(originalRainDrop->vbo, originalRainDrop->vbo, originalRainDrop->ibo, originalRainDrop->data, 1, 1);
...
//    float posX, posY, posZ;
	...
	//glBindVertexArray(originalRainDrop->vbo);
...
void BSTexture::renderCoins()
{
	...
	for(int it = 0; it < listOfCoins.size(); it++)
	{
		...
		if(!listOfCoins[it]->deleteIt)
		{
			...
			listOfCoins[it]->draw(originalCoin->texture, originalCoin->vbo);
		}
	}
}
...
void BSTexture::renderFans()
{
	for(int it = 0; it < listOfFans.size(); it++)
	{
        ...
        listOfFans[it]->draw(originalFan->texture, originalFan->vbo);

		if(listOfFans[it]->powerY)
        {
			glUniform4f( locationBSTextureColor, 0, 0, 0, 1 );

			//glBindVertexArray(originalFanParticleY->vbo);
			...
			if(listOfFans[it]->powerX)
			{
				...
				//glBindVertexArray(originalFanParticleX->vbo);
				glBindTexture(GL_TEXTURE_2D, originalFan->texture);
				glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_LINEAR_MIPMAP_LINEAR);
				glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_LINEAR_MIPMAP_LINEAR);
...
//	yCordR = hero->theBody->GetPosition().y;
	...
	renderBody(hero->theBody->GetPosition().x, hero->theBody->GetPosition().y, hero->zCord, 0, 0, 0,
							blankTexture, mainColor, originalDarkMenu->vbo, originalDarkMenu->width, originalDarkMenu->height, originalDarkMenu->depth);
	renderBody(hero->theBody->GetPosition().x, hero->theBody->GetPosition().y, hero->zCord, 0, 0, 0,
							blankTexture, mainColor, originalDarkMenu->vbo, originalDarkMenu->width, originalDarkMenu->height, originalDarkMenu->depth);
...
{
	//LOGI("Number of obj: %i", ListOfSquareBodies.size());
	for(int i = 0; i < ListOfSquareBodies.size(); i++)
	{
			...
			//	|| ListOfSquareBodies[i]->texture)
		{
			...
			//renderBody((*i)->theBody->GetPosition().x, (*i)->theBody->GetPosition().y,
            //                        (*i)->zCord, 0, 0, 0,
			//						(*i)->texture, (*i)->color, (*i)->vbo, (*i)->width, (*i)->height, (*i)->depth);
		}
	}
}
...
{
// 	renderBody(mouseTexture->xCord + CameraPosition->xCord - CameraExtremeLeft,
//                            mouseTexture->yCord + CameraPosition->yCord - CameraExtremeDown,
//                            scaleSizeInitial, 0, 0, 0,
//							mouseTexture->texture, mouseTexture->color, mouseTexture->vbo,
...
void BSTexture::renderTheObstacles()
{
	...
	for(int i = 0; i < ListOfParticles.size(); i++)
	{
		if(ListOfParticles[i]->isDestroyed == false)
		{
			...
			ListOfParticles[i]->draw(blankTexture, originalParticle->vbo);
...
void BSTexture::setCameraPosition()
{
    ...
    //CameraPosition->xCord = hero->theBody->GetPosition().x * cross_platform_scale;
    //CameraPosition->xCord -= SCREEN_WIDTH / 2.0f;

    //CameraPosition->yCord = hero->theBody->GetPosition().y * cross_platform_scale;
    //CameraPosition->yCord -= SCREEN_HEIGHT / 2.0f;
}
...
void BSTexture::renderLevelSceneChangeFromMenuLevels()
{
	if(deadScene->CosValueColorOfTheScene < 90 && deadScene->colorGoesUpOrDown == 1)
	{
		...
		if(deadScene->CosValueColorOfTheScene >=90 && deadScene->colorGoesUpOrDown == 1)
		{
			...
			std::string someFile = getFile(nameOfLevel.c_str());
			std::stringstream citesteLevelulCurent;
			citesteLevelulCurent << someFile;
...
void BSTexture::renderDustBodies()
{
    for(int i = 0; i < ListOfSquareDustBodies.size(); i++)
    {
        ...
        if(color.a > 0)
        {
            renderBody(ListOfSquareDustBodies[i]->xCord, ListOfSquareDustBodies[i]->yCord - decal, originalCoin->zCord, 0, 0, 0,
                                    originalDustBody->texture, color, originalDustBody->vbo,
...
void BSTexture::renderTheCoin()
{
	...
	renderBody(originalCoin->xCord + CameraPosition->xCord - CameraExtremeLeft, originalCoin->yCord + CameraPosition->yCord - CameraExtremeDown,
                            originalCoin->zCord, 0, 0, 0, originalCoin->texture, color, originalCoin->vbo, originalCoin->width, originalCoin->height, originalCoin->depth);
}


void BSTexture::renderWormHoles()
{
    BSColorRGBA color = {0, 0, 255, 1};
    ...
    if(!listOfWormHoles.empty())
    {
        for(int i = 0; i < listOfWormHoles.size(); i++)
        {
            ...
            renderBody(listOfWormHoles[i]->xCord, listOfWormHoles[i]->yCord, listOfWormHoles[i]->zCord, listOfWormHoles[i]->value1, 0, 0,
                                    originalWormHole->texture, color, originalWormHole->vbo,
...
{
//    CameraPosition->xCord += SCREEN_WIDTH / 2.0f;
//    CameraPosition->xCord /= cross_platform_scale;
//
//    CameraPosition->yCord += SCREEN_HEIGHT / 2.0f;
//    CameraPosition->yCord /= cross_platform_scale;

	//LOGI("%f %f %f %f", backBtn->x, CameraPosition->xCord, CameraExtremeLeft, hero->theBody->GetPosition().x);
	BSColorRGBA color = {1.0f, 1.0f, 1.0f, 1.0f};
	//renderBody(backBtn->x + CameraPosition->xCord + CameraExtremeLeft, hero->theBody->GetPosition().y, scaleSizeInitial, 0, 0, 0, backBtn->textureUp, color, backBtn->vbo, backBtn->w * 2, backBtn->h * 2, 0);
	if(backBtn->isDown == false)
	{
		renderBody(backBtn->x + CameraPosition->xCord - CameraExtremeLeft + backBtn->w / 2.0f,
				backBtn->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, backBtn->textureUp, color, backBtn->vbo, backBtn->w, backBtn->h, 0);
	}
	else
	{
		renderBody(backBtn->x + CameraPosition->xCord - CameraExtremeLeft + backBtn->w / 2.0f,
				backBtn->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, backBtn->textureDown, color, backBtn->vbo, backBtn->w, backBtn->h, 0);
	}
	...
	if(btnDarkLevels->isDown == false)
	{
		if(DarkLevels)
		{
			renderBody(btnDarkLevels->x + CameraPosition->xCord - CameraExtremeLeft + btnDarkLevels->w / 2.0f,
					btnDarkLevels->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnDarkLevels->textureDown, color, btnDarkLevels->vbo, btnDarkLevels->w, btnDarkLevels->h, 0);
		}
		else
		{
			renderBody(btnDarkLevels->x + CameraPosition->xCord - CameraExtremeLeft + btnDarkLevels->w / 2.0f,
					btnDarkLevels->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnDarkLevels->textureUp, color, btnDarkLevels->vbo, btnDarkLevels->w, btnDarkLevels->h, 0);
		}
	}
	else
	{
	...
	if(btnTimeControl->isDown == false)
	{
		...
		{
			renderBody(btnTimeControl->x + CameraPosition->xCord - CameraExtremeLeft + btnTimeControl->w / 2.0f,
					btnTimeControl->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnTimeControl->textureUp, color, btnTimeControl->vbo, btnTimeControl->w, btnTimeControl->h, 0);
		}
		else
		{
			renderBody(btnTimeControl->x + CameraPosition->xCord - CameraExtremeLeft + btnTimeControl->w / 2.0f,
					btnTimeControl->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnTimeControl->textureDown, color, btnTimeControl->vbo, btnTimeControl->w, btnTimeControl->h, 0);
		}
	}
	else
	{
    ...
    if(btnPowerUp1->isDown == false)
    {
		...
		{
			renderBody(btnPowerUp1->x + CameraPosition->xCord - CameraExtremeLeft + btnPowerUp1->w / 2.0f, btnPowerUp1->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnPowerUp1->textureUp, color, btnPowerUp1->vbo, btnPowerUp1->w, btnPowerUp1->h, 0);
		}
		else
		{
			renderBody(btnPowerUp1->x + CameraPosition->xCord - CameraExtremeLeft + btnPowerUp1->w / 2.0f, btnPowerUp1->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnPowerUp1->textureDown, color, btnPowerUp1->vbo, btnPowerUp1->w, btnPowerUp1->h, 0);
		}
    }
    else
    {
    ...
    if(btnPowerUp2->isDown == false)
    {
		...
		{
			renderBody(btnPowerUp2->x + CameraPosition->xCord - CameraExtremeLeft + btnPowerUp2->w / 2.0f, btnPowerUp2->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnPowerUp2->textureUp, color, btnPowerUp2->vbo, btnPowerUp2->w, btnPowerUp2->h, 0);
		}
		else
		{
			renderBody(btnPowerUp2->x + CameraPosition->xCord - CameraExtremeLeft + btnPowerUp2->w / 2.0f, btnPowerUp2->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnPowerUp2->textureDown, color, btnPowerUp2->vbo, btnPowerUp2->w, btnPowerUp2->h, 0);
		}
    }
    else
    {
    ...
    if(btnPowerUp3->isDown == false)
    {
		...
		{
			renderBody(btnPowerUp3->x + CameraPosition->xCord - CameraExtremeLeft + btnPowerUp3->w / 2.0f, btnPowerUp3->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnPowerUp3->textureUp, color, btnPowerUp3->vbo, btnPowerUp3->w, btnPowerUp3->h, 0);
		}
		else
		{
			renderBody(btnPowerUp3->x + CameraPosition->xCord - CameraExtremeLeft + btnPowerUp3->w / 2.0f, btnPowerUp3->y + CameraPosition->yCord - CameraExtremeDown, scaleSizeInitial, 0, 0, 0, btnPowerUp3->textureDown, color, btnPowerUp3->vbo, btnPowerUp3->w, btnPowerUp3->h, 0);
		}
    }
    else
    {
...
void BSTexture::renderMenuLevels(int nrOfLevels, ChapterLevelsStructure* theLevelsToRender[])
{
	...
	for(int i = 0 ; i < nrOfLevels ; i++)
	{
		...
		if( (i + 1) % 5 != 0)
		{
			...
			renderBody(theLevelsToRender[i]->xCord + CameraPosition->xCord - CameraExtremeLeft + theLevelsToRender[i]->width / 2.f + theLevelsToRender[i]->width / 2.f,
									...
									theLevelsToRender[i + 1]->color, theLevelsToRender[0]->vboBufferOfConnectionLeftRight,
		...
		if(i < 20)
		{
			...
			renderBody(theLevelsToRender[i]->xCord + CameraPosition->xCord - CameraExtremeLeft,
									...
									theLevelsToRender[i + 5]->color, theLevelsToRender[0]->vboBufferOfConnectionUpDown,
	...
	for(int i = 0 ; i < nrOfLevels ; i++)
	{
		...
		renderBody(theLevelsToRender[i]->xCord + CameraPosition->xCord - CameraExtremeLeft,
								...
								theLevelsToRender[i]->color, theLevelsToRender[i]->vboBufferOfLevel,
		...
		if(theLevelsToRender[i]->isFinished)
		{
			...
			renderBody(theLevelsToRender[i]->xCord+ CameraPosition->xCord - CameraExtremeLeft,
									...
									theLevelsToRender[0]->color, theLevelsToRender[0]->vboBufferOfFinishedLevel,
									theLevelsToRender[0]->width, theLevelsToRender[0]->height, theLevelsToRender[0]->depth);
		}
	}
	...
	renderBody(levelChoserPlayer->xCord+ CameraPosition->xCord - CameraExtremeLeft,
                         ...
                         levelChoserPlayer->vbo, levelChoserPlayer->width, levelChoserPlayer->height,
...
void BSTexture::renderMenuChapters()
{
	...
	for(int i = 0 ; i < nrOfChapters - 1 ; i++)
	{
		...
		{
			renderBody(chaptere[i]->xCord + CameraPosition->xCord - CameraExtremeLeft + chaptere[i]->width / 2.f + sizeOfConnectionStringX / 2.f + 0.1,
									...
									chaptere[indexulUrmatoruluiNivel]->color, chaptere[0]->levelFromChapter[0]->vboBufferOfConnectionLeftRightChapter,
			...
			if(chaptere[indexulUrmatoruluiNivel]->xCord < chaptere[i]->xCord)
			{
				renderBody(chaptere[i]->xCord + CameraPosition->xCord - CameraExtremeLeft - chaptere[i]->width / 2.f - sizeOfConnectionStringX / 2.f - 0.12,
										chaptere[i]->yCord + CameraPosition->yCord - CameraExtremeDown,
										scaleSizeInitial, 0, 0, 0,
				...
				{
					renderBody(chaptere[i]->xCord + CameraPosition->xCord - CameraExtremeLeft,
		...
		if(chaptere[i]->isFinishedChapter)
		{
			renderBody(chaptere[i]->xCord+ CameraPosition->xCord - CameraExtremeLeft,
                           ...
                           scaleSizeInitial, 0, 0, 0,
									...
									chaptere[0]->levelFromChapter[0]->vboBufferOfFinishedLevel, 0, 0, 0);
		}
	}
	...
	renderBody(levelChoserPlayer->xCord + CameraPosition->xCord - CameraExtremeLeft,
							levelChoserPlayer->yCord + CameraPosition->yCord - CameraExtremeDown,
							scaleSizeInitial, 0, 0, 0,
							levelChoserPlayer->texture, levelChoserPlayer->color, levelChoserPlayer->vbo,
...
void BSTexture::renderMessages()
{
	...
	if(nrOfMessageSign)
	{
		...
		{
			///Randez semnul
			renderBody(listOfMessages[i]->xCord, listOfMessages[i]->yCord, scaleSizeInitial, 0, 0, 0,
									listOfMessages[i]->textureOfSign, listOfMessages[i]->colorOfSign,
									listOfMessages[i]->vboBufferOfSign, listOfMessages[i]->widthOfSign, listOfMessages[i]->heightOfSign, 0);
			...
			if(listOfMessages[i]->isShownTheMessage)
			{
				...
				renderBody(listOfMessages[i]->xCord + listOfMessages[i]->widthOfMessage / 2.f,
										...
										listOfMessages[i]->vboBufferOfMessage, listOfMessages[i]->widthOfMessage, listOfMessages[i]->heightOfMessage, 0);
			}
			else
			{
				///Il fac transparent
...
void BSTexture::renderTransparentSquare(BSColorRGBA theColorToRenders, float coordinateX, float coordinateY)
{
	...
	renderBody(coordinateX, coordinateY, scaleSizeInitial, 0, 0, 0, whiteTexture,
							theColorToRenders, originalTransparentMenu->vbo,
...
void BSTexture::renderAnyButtons(BSMenuStructure *theMenu)
{
	...
	for(int i = 0; i < theMenu->listOfButtons.size(); i++)
	{
		...
		{
			renderBody(currentButton->xCord + CameraPosition->xCord - CameraExtremeLeft,
                                    currentButton->yCord + CameraPosition->yCord - CameraExtremeDown,
                                    scaleSizeInitial, 0, 0, 0, currentButton->textureOfButtons[1], currentButton->color, currentButton->vbo[1],
									newW, newH, currentButton->depth);
		}
		else
		{
			renderBody(theMenu->listOfButtons[i]->xCord + CameraPosition->xCord - CameraExtremeLeft,
                                    ...
                                    theMenu->listOfButtons[i]->vbo[0], theMenu->listOfButtons[i]->width,
                                    theMenu->listOfButtons[i]->height, theMenu->listOfButtons[i]->depth);
		}
...
void BSTexture::renderSoundAnimation(float yCord, float theVolume)
{
	...
	{
		renderBody(SoundButtonTexture->xCord + CameraPosition->xCord - CameraExtremeLeft + Hero_size * i + 0.25 * i - 3.2,
								yCord + CameraPosition->yCord - CameraExtremeDown,
								scaleSizeInitial, 0, 0, 0, SoundButtonTexture->texture, SoundButtonTexture->color,
								SoundButtonTexture->vbo, SoundButtonTexture->width, SoundButtonTexture->height, SoundButtonTexture->depth);
	}
}
...
void BSTexture::renderBackgroundNo1()
{
	...
	renderBody(backgroundSky->xCord - 1, backgroundSky->yCord - 2, -40, 0, 0, 0,
							backgroundSky->texture, backgroundSky->color, backgroundSky->vbo,
							2.4f * backgroundSky->width, 3 * backgroundSky->height + 10, 0);

	renderBody(backgroundMountain->xCord, backgroundMountain->yCord, -35, 0, 0, 0,
							backgroundMountain->texture, backgroundMountain->color, backgroundMountain->vbo,
							2 * backgroundMountain->width, 3 * backgroundMountain->height, 0);

	renderBody(backgroundBush->xCord, backgroundBush->yCord, -30, 0, 0, 0,
							backgroundBush->texture, backgroundBush->color, backgroundBush->vbo,
							2 * backgroundBush->width, 2 * backgroundBush->height, 0);

	renderBody(backgroundHighField->xCord - 10, backgroundHighField->yCord, -23, 0, 0, 0,
							backgroundHighField->texture, backgroundHighField->color, backgroundHighField->vbo,
							2 * backgroundHighField->width, 2 * backgroundHighField->height, 0);
}


/*void BSTexture::renderBody(float coordX, float coordY, float coordZ, float angleX, float angleY, float angleZ,
                                        GLuint theTextureIdToRender, BSColorRGBA theColorOfObstacle,
                                        GLuint thevboBuffer, float ScaleOnX, float ScaleOnY, float ScaleOnZ)
{
...
//    BSModelViewMatrix = glm::rotate( worldAngle, 0.0f, 0.0f, 1.0f ) * BSModelViewMatrix;
	...
	//bind the vbo buffer, and draw the objects
	//glBindVertexArray( thevboBuffer );
...
void BSTexture::loadTextures()
{
    ...
    loadTexture("whiteImage.png", whiteTexture);
	loadTexture("Animations/roundAnimation/windmillPropeller.png", textureMill);
	loadTexture("playerTexture.png", playerTexture);
	//loadTexture("playerTexture round.png", playerTextureRound);
	//loadTexture("playerTexture Dead.png", playerTextureDead);
	loadTexture("background.png", backgroundTexture);
}
...
void BSTexture::initTheFont()
{
	...
	empty.push_back("imagePath");
	empty.push_back("width");
	empty.push_back("height");

	doc->readAnyXML("Animations/Font/dataOfFont.xml", "TextureAtlas", "sprite", theNodesNew, empty, levelDetailsNew, theAttrNew);
	...
	while(i < levelDetailsNew.size() && levelDetailsNew[i])
	{
		...
		if(!i)
		{
			loadTexture("Animations/Font/dataOfFont.png", letters[0]->texture);
		}

        letters[let]->color.setColor(0, 0, 0, 1);

		addObstacle->setTheVaoDataForFont(letters[let]->data, letters[let]->widthOfLetterInImage, letters[let]->heightOfLetterInImage, letters[let]->xCordInImage, letters[let]->yCordInImage, letters[let]->vbo, letters[let]->vbo, letters[let]->ibo);
	...
	for(unsigned int i = 0 ; i < theStringToRenderString.size() ; i++)
	{
		if(letters[theStringToRenderString[i]] || theStringToRenderString[i] == ' ')
		{
			...
			if( (theStringToRenderString[i] >= 'a' && theStringToRenderString[i] <= 'z') || (theStringToRenderString[i] >= 'A' && theStringToRenderString[i] <= 'Z') || (theStringToRenderString[i] >= '0' && theStringToRenderString[i] <= '9'))
			{
				...
				lastX = pozNou;
				lastWidth = newWidth;
			    checkGlError("glFont1");
				renderBody(pozNou, newY, scaleSizeInitial, 0, 0, 0, letters[0]->texture, newColor, letters[let]->vbo, xSize, ySize, 0);
			    checkGlError("glFont2");
		}
	}
}
...
//	BSColorRGBA color = {0, 0, 0, 1};
	...
	//glBindVertexArray(originalCoin->vbo);

...
bool BSUtilities::loadGP(int w, int h)
{
	SCREEN_WIDTH = w;
	SCREEN_HEIGHT = h;
...
bool BSUtilities::loadMedia()
{
 //       SDL_PollEvent( &event );
    ...
    checkGlError("glMedia1");
	//LOGI("%s", "hello1");
	obstacle->setTheVaoData(originalBody->vbo, originalBody->vbo, originalBody->ibo, originalBody->data, 1.0f, 1.0f);
	//LOGI("%s", "hello2");
    checkGlError("glMedia2");

	gOpenGLTexture->loadTheLoadingImage();
	//LOGI("%s", "hello3");
    checkGlError("glMedia3");


	glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	//LOGI("%s", "hello4");
    checkGlError("glMedia4");
	gOpenGLTexture->renderTheLoadingImage(1);
	//LOGI("%s", "hello5");
    checkGlError("glMedia5");
	//SDL_GL_SwapBuffers();

	loadTexture("blank.png", blankTexture);

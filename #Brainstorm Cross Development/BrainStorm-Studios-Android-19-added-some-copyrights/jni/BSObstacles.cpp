...
void BSObstacles::setTheVaoData(GLuint &vaoBuffer, GLuint &vboBuffer, GLuint &iboBuffer, BSTexturedVertex2D dataToSet[4], GLfloat widthToSet, GLfloat heightToSet)
{
	widthToSet = 1 / 2.0f;
	heightToSet = 1 / 2.0f;


	GLfloat texTop = 0;//heightToSet * 2;
	GLfloat texBottom = 1;
	GLfloat texLeft = 0;
...
void BSObstacles::setTheVaoDataForFont(BSTexturedVertex2D dataTSet[4], GLfloat widthToSet, GLfloat heightToSet, float xCord, float yCord, GLuint &vboBuffer, GLuint &vaoBuffer, GLuint &iboBuffer)
{
...
//        listOfActiveAnimations[i]->canPlayAnimation3 = false;
	...
	setTheVaoData(originalParticle->vbo, originalParticle->vbo, originalParticle->ibo, originalParticle->data, 1  / 16.0f, 1  / 16.0f);
	...
	setTheVaoData(Hero2nd->vbo, Hero2nd->vbo, Hero2nd->ibo, Hero2nd->data, Hero2nd->width, Hero2nd->height);
	...
	setTheVaoData(SoundButtonTexture->vbo, SoundButtonTexture->vbo, SoundButtonTexture->ibo,
				  SoundButtonTexture->data, SoundButtonTexture->width, SoundButtonTexture->height);



	///Set the coin
	loadTexture("Animations/coin.png", originalCoin->texture);
	...
	setTheVaoData(originalCoin->vbo, originalCoin->vbo, originalCoin->ibo, originalCoin->data, 1, 1);


	///Set the dust bodies
	loadTexture("Animations/dust.png", originalDustBody->texture);
	originalDustBody->width = 1;
	originalDustBody->height = 1;
	originalDustBody->xCord = 0;
	originalDustBody->yCord = 0;
	setTheVaoData(originalDustBody->vbo, originalDustBody->vbo, originalDustBody->ibo, originalDustBody->data, 1, 1);


	///Set the fans
	loadTexture("Animations/fan.png", originalFan->texture);
	originalFan->width = Hero_size * 10;
	originalFan->height = Hero_size;
	setTheVaoData(originalFan->vbo, originalFan->vbo, originalFan->ibo, originalFan->data, 1, 1);
	setVaoForChunckObjects(originalFanParticleX->vbo, originalFanParticleX->vbo, originalFanParticleX->ibo, originalFanParticleX->data, 1, 0.05f);
	setVaoForChunckObjects(originalFanParticleY->vbo, originalFanParticleY->vbo, originalFanParticleY->ibo, originalFanParticleY->data, 0.05f, 1);


	///Set the dark level light
	//loadTexture("Animations/dark.png", mainDarkTexture);
	originalDarkMenu->width = (float) (SCREEN_WIDTH / scaleRatio + 2) * 2;
	originalDarkMenu->height = (float) (SCREEN_HEIGHT / scaleRatio + 2) * 2;
	setTheVaoData(originalDarkMenu->vbo, originalDarkMenu->vbo, originalDarkMenu->ibo, originalDarkMenu->data, 1, 1);
	...
	setTheVaoData(originalDarkLevel->vbo, originalDarkLevel->vbo, originalDarkLevel->ibo, originalDarkLevel->data, 1, 1);


	originalWormHole->width = 2;
	originalWormHole->height = 2;
	setTheVaoData(originalWormHole->vbo, originalWormHole->vbo, originalWormHole->ibo, originalWormHole->data, 1, 1);
	loadTexture("Animations/roundAnimation/windmillPropeller2.png", originalWormHole->texture);
	...
	setTheVaoData(theRenderedObstacleSquare->vbo, theRenderedObstacleSquare->vbo, theRenderedObstacleSquare->ibo,
	...
	setTheVaoData(originalTransparentMenu->vbo, originalTransparentMenu->vbo, originalTransparentMenu->ibo,
	...
	setTheVaoData(theRenderedObstacleCircle->vbo, theRenderedObstacleCircle->vbo, theRenderedObstacleCircle->ibo,
	...
	deadScene->texture = whiteTexture;
	setTheVaoData(deadScene->vbo, deadScene->vbo, deadScene->ibo, deadScene->data, 1, 1);


	int xF = 4, yF = originalCoin->yCord;
	backBtn->x = xF;
	backBtn->y = yF;
	backBtn->w = 2;
	backBtn->h = 2;
	...
	setTheVaoData(backBtn->vbo, backBtn->vbo, backBtn->ibo, backBtn->data, backBtn->w, backBtn->h);
	loadTexture("Animations/pauseDown.png", backBtn->textureDown);
	loadTexture("Animations/pauseUp.png", backBtn->textureUp);
	...
	setTheVaoData(btnPowerUp1->vbo, btnPowerUp1->vbo, btnPowerUp1->ibo, btnPowerUp1->data, btnPowerUp1->w, btnPowerUp1->h);
	loadTexture("Animations/flame1.png", btnPowerUp1->textureDown);
	loadTexture("Animations/flame1.png", btnPowerUp1->textureUp);
	...
	setTheVaoData(btnPowerUp2->vbo, btnPowerUp2->vbo, btnPowerUp2->ibo, btnPowerUp2->data, btnPowerUp2->w, btnPowerUp2->h);
	loadTexture("Animations/flame2.png", btnPowerUp2->textureDown);
	loadTexture("Animations/flame2.png", btnPowerUp2->textureUp);
	...
	setTheVaoData(btnPowerUp3->vbo, btnPowerUp3->vbo, btnPowerUp3->ibo, btnPowerUp3->data, btnPowerUp3->w, btnPowerUp3->h);
	loadTexture("Animations/flame3.png", btnPowerUp3->textureDown);
	loadTexture("Animations/flame3.png", btnPowerUp3->textureUp);


	btnDarkLevels->x = xF + 16;
	btnDarkLevels->y = yF;
	btnDarkLevels->w = 2;
	btnDarkLevels->h = 2;
	btnDarkLevels->isDown = false;
	btnDarkLevels->canClick = false;
	setTheVaoData(btnDarkLevels->vbo, btnDarkLevels->vbo, btnDarkLevels->ibo, btnDarkLevels->data, btnDarkLevels->w, btnDarkLevels->h);
	loadTexture("Animations/darkDown.png", btnDarkLevels->textureDown);
	loadTexture("Animations/darkUp.png", btnDarkLevels->textureUp);


	btnTimeControl->x = xF + 20;
	btnTimeControl->y = yF;
	btnTimeControl->w = 2;
	btnTimeControl->h = 2;
	btnTimeControl->isDown = false;
	btnTimeControl->canClick = false;
	setTheVaoData(btnTimeControl->vbo, btnTimeControl->vbo, btnTimeControl->ibo, btnTimeControl->data, btnTimeControl->w, btnTimeControl->h);
	loadTexture("Animations/timeDown.png", btnTimeControl->textureDown);
	loadTexture("Animations/timeUp.png", btnTimeControl->textureUp);
}
...
void BSObstacles::incarcaHarta(std::string cale)
{
	...
	std::string myFile = getFile("playerSize.txt");
	std::stringstream playerSize;
	playerSize<<myFile;
	...
	if(hero->width != Hero_size && hero->height != Hero_size)
	{
		...
		setTheVaoData(hero->vbo, hero->vbo, hero->ibo, hero->data, playerAnimWidth, playerAnimHeight);
...
void BSObstacles::addTheAnimationsForMap(std::string theLocationOfAnimation)
{
	...
	std::string myFile = getFile(newAnimationDetailPath.c_str());
	if(myFile != "")
	{
		std::stringstream animationsDetails;
		animationsDetails<<myFile;
...
//		LOGI("Nr of animations: %i", totalNrAnimations);
...
//			LOGI("%i, %i, %i, %i, %i, %i, %i, %i, %i, %s", total1, total2, total3, totalTime1, totalTime2, totalTime3, totalLoop1, totalLoop2, totalLoop3, detailName.c_str());

			std::string newAnimationCale = "Animations/" + theLocationOfAnimation + "/" + detailName + "/playerTexture";
//			LOGI("anim add: %s", newAnimationCale.c_str());
...
//			LOGI("anim done add: %s", newAnimationCale.c_str());
		}
		nrOfListOfAnimations = 1;
	}
	else
	{
		nrOfListOfAnimations = 0;
	}
	LOGI("%s", "animations added");
}
								   ...
								   b2BodyType tipulCorpului, int canKillHero, int isDust)
{
	...
	LOGI("%s %i, %f, %f, %f, %f, %s ", "new square Body:", canKillHero, BodyX, BodyY, BodyW, BodyH, userData.c_str());
	...
	setTheVaoData(p->vbo, p->vbo, p->ibo, p->data, BodyW, BodyH);
	...
	p->texture = texture;
	loadTexture(textureName, p->texture);
								  ...
								  b2BodyType tipulCorpului, int canKillHero)
{
	...
	setTheVaoData(p->vbo, p->vbo, p->ibo, p->data, BodyR, BodyR);
	p->width = BodyR;
	p->height = BodyR;
	p->nameOfTexture = textureName;
	p->script = NULL;
								 ...
								 float width, float height, std::string texturePath)
{
	if(squareOrCirlce == 1)
	{
		...
		setTheVaoData(p->vbo, p->vbo, p->ibo, p->data, width, height);
		p->width = width;
		p->height = height;
		p->xCord = xCoord;
		p->yCord = yCoord;
		...
		loadTexture(textureCharPath, p->texture);
	...
	else if(squareOrCirlce == -1)
	{
		...
		setTheVaoData(p->vbo, p->vbo, p->ibo, p->data, width, width);
		p->width = width;
		p->height = width;
		p->xCord = xCoord;
		p->yCord = yCoord;
		...
		loadTexture(texturePath.c_str(), p->texture);
...
void BSObstacles::addMessageTexture(float xCoord, float yCoord, float width, float height, std::string textureName)
{
	...
	loadTexture(textureName, newMessage->textureOfMessage);
	...
	setTheVaoData(newMessage->vboBufferOfMessage, newMessage->vboBufferOfMessage, newMessage->iboBufferOfMessage,
	...
	loadTexture(fullTextureName.c_str(), newMessage->textureOfSign);
	setTheVaoData(newMessage->vboBufferOfSign, newMessage->vboBufferOfSign, newMessage->iboBufferOfSign,
...
void BSObstacles::addSnowFlakeS()
{
	...
	for(int i = 0 ; i < 150; i++)
	{
		...
		setVaoForChunckObjects(newFlake->vbo, newFlake->vbo, newFlake->ibo, newFlake->data, newFlake->width, newFlake->width);
...
void BSObstacles::createMouseCircles()
{
	...
	for(int i = 1 ; i < 15 ; i++)
	{
		...
		circ->xCord = 0;
		circ->yCord = 0;
		setTheVaoData(circ->vbo, circ->vbo, circ->ibo, circ->data, circ->width, circ->width);
...
void BSObstacles::addBackgrounds()
{
	...
	loadTexture("Background images/sky.png", backgroundSky->texture);
	setTheVaoData(backgroundSky->vbo, backgroundSky->vbo, backgroundSky->ibo,
	...
	loadTexture("Background images/mountains.png", backgroundMountain->texture);
	setTheVaoData(backgroundMountain->vbo, backgroundMountain->vbo, backgroundMountain->ibo,
	...
	loadTexture("Background images/bush.png", backgroundBush->texture);
	setTheVaoData(backgroundBush->vbo, backgroundBush->vbo, backgroundBush->ibo,
	...
	loadTexture("Background images/field.png", backgroundField->texture);
	setTheVaoData(backgroundField->vbo, backgroundField->vbo, backgroundField->ibo,
	...
	loadTexture("Background images/high field.png", backgroundHighField->texture);
	setTheVaoData(backgroundHighField->vbo, backgroundHighField->vbo, backgroundHighField->ibo,
...
void BSObstacles::addCloudyParticles()
{
	...
	for(int i = 0; i < 10; i++)
	{
		...
		setTheVaoData(part->vbo, part->vbo, part->ibo, part->data, part->width, part->height);
...
void BSObstacles::addTheTexturesOfCloudyParticles()
{
	for(int i = 1; i <= 5; i++)
	{
		loadTexture("Background images/cloud" + BStoString(i) + ".png", clouds[i]);
	}
}

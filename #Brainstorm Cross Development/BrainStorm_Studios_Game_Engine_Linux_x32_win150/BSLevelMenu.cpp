...
//            std::cout<<name1Char<<' '<<name2Char<<' '<<name3Char<<'\n';
        ...
        if(i % 5 == 0)
        {
            ...
            theLevelsToAdd[i]->xCord = (float) SCREEN_WIDTH / 2 / 32 - 2 * (theLevelsToAdd[i]->width + spaceBetweenLevels);//2;
            ...
            {
                theLevelsToAdd[i]->yCord = (float) SCREEN_HEIGHT / 2 / 32 + 2 * (theLevelsToAdd[i]->height + spaceBetweenLevels);//SCREEN_HEIGHT / 32 - 2;
            }
        }
        else
        {
        ...
        ///renderLevelClass->loadATexture(nameOfTheTextureChar, theLevelsToAdd[i]->textureOfLevel);
...
//        if(!i)
		{
			renderLevelClass->loadATexture("Menu Of Chapters/connectionLeftRight.png",
										chaptere[i]->levelFromChapter[0]->textureOfConnectionLeftRightChapter);


			renderLevelClass->loadATexture("Menu Of Chapters/connectionUpDown.png",

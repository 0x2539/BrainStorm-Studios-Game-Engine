...
void BSTexture::renderTheObstacles()
{
	...
	for(iteratorParticles = ListOfParticles.begin(); iteratorParticles != ListOfParticles.end(); iteratorParticles++)
	{
		if((*iteratorParticles)->isDestroyed == false)
		{
			...
			{
				(*iteratorParticles)->color = { (*iteratorParticles)->color.r + 0.15,
									  (*iteratorParticles)->color.g + 0.15, 0.f, 1.f
														};
			}
...
void BSTexture::renderSnowFlakes()
{
	...
	for(std::list<BSSnowFlakes*>::iterator it = listOfSnowFlakes.begin(); it != listOfSnowFlakes.end(); it++)
	{
...
//		}
		...
		renderChuncksOfObjects((*it)->xCord, (*it)->yCord, scaleSizeInitial, (*it)->radius, (*it)->radius);

...
bool BSXmlFiles::writeAnyXML(std::string fileName, std::string headNode, std::string secondNode, std::vector<std::string> nodesNames,
                             std::vector<ListaDeCaractere*>nodesValues, std::vector<std::string> attributesNames, std::vector<std::string> attributesValues)
{
	std::string myNewFile = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
	myNewFile += "<" + headNode;
	for(int i = 0; i < attributesNames.size(); i++)
	{
		myNewFile += " " + attributesNames[i] + "=" + '"' + attributesValues[i] + '"';
	}
	myNewFile += ">\n";
	for(int i = 0; i < nodesValues.size(); i++)
	{
		myNewFile += "<" + secondNode + ">\n";

		for(int j = 0; j < nodesNames.size(); j++)
		{
			myNewFile += "<" + nodesNames[j] + ">";
			myNewFile += nodesValues[i]->theCharArray2D[j];
			myNewFile += "</" + nodesNames[j] + ">\n";
		}
